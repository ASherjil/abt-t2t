...
std::vector<std::vector<std::byte>> makeDay(std::uint64_t spanNs, int steps) {
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        if (roll < 30 && !live.empty()) {
            ...
            const Price tick = buy ? 31999 - static_cast<Price>(rnd() % 10) : 32001 + static_cast<Price>(rnd() % 10);
            live.push_back(va.injectSynthetic(buy ? Side::Buy : Side::Sell, tick, 100, ts));
        }
        for (std::size_t k = before; k < aapl.md.size(); ++k) out.push_back(aapl.md[k]);
        if (i % 5 == 0) {
            const std::size_t b = other.md.size();
            vo.injectSynthetic(Side::Sell, 3000, 10, ts);
            for (std::size_t k = b; k < other.md.size(); ++k) out.push_back(other.md[k]);
        }
    }
...
std::string writeDay(const std::vector<std::vector<std::byte>>& msgs, const char* name) {
    ...
    CHECK(w.ok());
    for (const auto& m : msgs) w.write(m);
    return path;
}
...
void test_afap_forwards_verbatim_and_loops() {
    ...
    std::size_t pumps = 0;
    while (rp.pump(monotonicNs())) ++pumps;
...
void test_paced_replay_takes_virtual_time() {
    ...
    while (rp.pump(monotonicNs())) {}
    const auto el = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t0).count();
    CHECK(el >= 38);
    CHECK(el < 400);
...
void test_client_order_survives_replay_and_reset_cancels_it() {
    ...
    for (const auto& p : ex.capturedOrderEntry()) {
        ...
        if (!sp.payload.empty() && static_cast<char>(sp.payload[0]) == 'C') ++canceled;
    }
    ...
    for (const auto& m : got) {
        if (static_cast<char>(m[0]) == 'A') {
            ...
            if (a.orderRef.value() >= (1ull << 62)) ++clientAdds;
        }
    }
