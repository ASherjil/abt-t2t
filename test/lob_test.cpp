//
// lob_test.cpp -- matching-engine invariants: price-time priority, multi-level sweeps,
// partial fills, cancels (incl. mid-FIFO), reduce, best-quote maintenance, and share
// conservation / no-crossed-book after matching.
//
#include <vector>
...
// helper: total shares across all recorded trades
...
void test_rest_only_no_cross() {
    ...
    CHECK(book.bestBid() < book.bestAsk());   // not crossed
}
...
void test_full_aggressor_fill() {
    ...
    CHECK(rem == lob::kNilHandle);            // aggressor fully filled, nothing rested
    CHECK_EQ(book.bestAsk(), lob::kNoPrice);  // book empty on the ask side
    CHECK(book.empty());
}
...
void test_price_time_priority() {
    lob::OrderBook book(1, 1000);
    Rec sink;
    book.add(1, Side::Sell, 52, 100, sink);   // first in time at 52
    book.add(2, Side::Sell, 52, 100, sink);   // second in time at 52
    book.add(99, Side::Buy, 52, 150, sink);   // sweeps 150

    CHECK_EQ(sink.trades.size(), 2u);
    CHECK_EQ(sink.trades[0].restingId, 1u);   // oldest first
    CHECK_EQ(sink.trades[0].qty, 100u);
    CHECK(sink.trades[0].restingFilled);
    CHECK_EQ(sink.trades[1].restingId, 2u);
    CHECK_EQ(sink.trades[1].qty, 50u);        // partial
...
void test_multilevel_sweep_price_improvement() {
    ...
    book.add(99, Side::Buy, 53, 150, sink);   // takes 52 first (best), then 53

    CHECK_EQ(sink.trades.size(), 2u);
    CHECK_EQ(sink.trades[0].price, 52);       // best price first
    CHECK_EQ(sink.trades[0].qty, 100u);
    ...
    CHECK_EQ(filled(sink), 150u);             // conservation
}
...
void test_aggressor_partial_rests() {
    ...
    Rec sink;
    book.add(1, Side::Sell, 52, 100, sink);
    const lob::Handle rem = book.add(99, Side::Buy, 52, 150, sink);  // 100 fills, 50 rests

    CHECK_EQ(sink.trades.size(), 1u);
    CHECK_EQ(sink.trades[0].qty, 100u);
    CHECK(rem != lob::kNilHandle);
    CHECK_EQ(book.bestAsk(), lob::kNoPrice);
    CHECK_EQ(book.bestBid(), 52);             // remainder rests as a bid
...
void test_cancel_and_best_update() {
    ...
    CHECK(book.empty());
    CHECK_EQ(book.cancel(h), 0u);             // double cancel is a no-op
}
...
void test_cancel_middle_of_fifo() {
    ...
    CHECK_EQ(book.cancel(hb), 100u);          // remove the middle order
    CHECK_EQ(book.volumeAt(Side::Sell, 50), 200u);

    Rec sink;
    book.add(99, Side::Buy, 50, 250, sink);   // should hit id 1 then id 3, skipping 2
    CHECK_EQ(sink.trades.size(), 2u);
    CHECK_EQ(sink.trades[0].restingId, 1u);
...
void test_reduce() {
    lob::OrderBook book(1, 1000);
    const lob::Handle h = book.add(1, Side::Buy, 50, 100);
    CHECK_EQ(book.reduce(h, 60), 40u);        // 100 -> 60
    CHECK_EQ(book.volumeAt(Side::Buy, 50), 60u);
    CHECK_EQ(book.reduce(h, 80), 0u);         // increases ignored
    CHECK_EQ(book.volumeAt(Side::Buy, 50), 60u);
    CHECK_EQ(book.reduce(h, 0), 60u);         // reduce-to-zero cancels
    CHECK_EQ(book.bestBid(), lob::kNoPrice);
}
