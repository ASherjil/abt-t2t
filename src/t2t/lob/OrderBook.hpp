...
inline void OrderBook::clear() noexcept {
    for (Level& l : m_bidLevels) {
        l = Level{};
    }
    for (Level& l : m_askLevels) {
        l = Level{};
    }
    m_pool.clear();
...
inline Quantity OrderBook::cancel(Handle h) {
    ...
    free(h);
    if (lvl.totalQty == 0) {
        if (s == Side::Buy && p == m_bestBid) {
            rescanBestBid();
        } else if (s == Side::Sell && p == m_bestAsk) {
            rescanBestAsk();
        }
    }
    return removed;
...
inline Price OrderBook::bestBid() const noexcept {
    return m_bestBid;
}

inline Price OrderBook::bestAsk() const noexcept {
    return m_bestAsk;
}
...
inline const Order& OrderBook::order(Handle h) const noexcept {
    return m_pool[h];
}
...
inline Price OrderBook::minPrice() const noexcept {
    return m_minPrice;
}

inline Price OrderBook::maxPrice() const noexcept {
    return m_maxPrice;
}
...
inline void OrderBook::unlink(Level& lvl, Handle h) noexcept {
    ...
    if (o.next != kNilHandle) {
        m_pool[o.next].prev = o.prev;
    } else {
        lvl.tail = o.prev;
    }
}
...
Quantity OrderBook::matchLevel(Level& lvl, OrderId aggId, Side aggSide, Quantity qty, Sink& sink) {
    while (qty > 0 && lvl.head != kNilHandle) {
        ...
        if (restingFilled) {
            lvl.head = r.next;
            if (lvl.head != kNilHandle) {
                m_pool[lvl.head].prev = kNilHandle;
            } else {
                lvl.tail = kNilHandle;
            }
            --lvl.count;
...
inline void OrderBook::rescanBestBid() noexcept {
    for (Price p = m_bestBid; p >= m_minPrice; --p) {
        if (m_bidLevels[index(p)].totalQty > 0) {
            m_bestBid = p;
            return;
        }
    }
...
inline void OrderBook::rescanBestAsk() noexcept {
    for (Price p = m_bestAsk; p <= m_maxPrice; ++p) {
        if (m_askLevels[index(p)].totalQty > 0) {
            m_bestAsk = p;
            return;
        }
    }
